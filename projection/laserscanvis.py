...
class LaserScanVis:
    ...
    def reset(self):
        ...
        self.canvas = SceneCanvas(keys='interactive', show=True)  # 创建场景
        # interface (n next, b back, q quit, very simple)
        self.canvas.events.key_press.connect(self.key_press)  # 添加事件
        self.canvas.events.draw.connect(self.draw)  # 添加事件

        # grid
        self.grid = self.canvas.central_widget.add_grid()  # 添加一个格子

        # laserscan part
        self.scan_view = vispy.scene.widgets.ViewBox(border_color='white', parent=self.canvas.scene)  # 添加一个控件
        self.grid.add_widget(self.scan_view, 0, 0)  # 第0行第0列的格子显示控件
        self.scan_vis = visuals.Markers()  # Visual displaying marker symbols
        self.scan_view.camera = 'turntable'  # 相机类型='turntable'
        self.scan_view.add(self.scan_vis)
        visuals.XYZAxis(parent=self.scan_view.scene)  # 添加坐标轴
        # add semantics
        if self.semantics:
