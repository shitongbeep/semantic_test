...
class LaserScan:
    ...
    def open_scan(self, filename, scan_unfold=True):
        ...
        self.set_points(points, remissions, scan_unfold=scan_unfold)

    def set_points(self, points, remissions=None, scan_unfold=True):
        ...
        if self.project and scan_unfold:
            self.scan_unfold_projection()
        elif self.project and not scan_unfold:
            self.do_range_projection()

    def scan_unfold_projection(self):
        """ Scan unfolding aims to mitigate the problems due to the alternate
        spherical projection method which suffers from point occlusions due to
        ego-motion correction. Scan unfolding yields a much denser representation 
        than spherical projection."""
        ...
        np.savetxt('./saveprojx.txt', proj_x)

        jump = np.nonzero((proj_x[1:] < 0.2) * (proj_x[:-1] > 0.8))[0] + 1
        proj_y = np.zeros_like(proj_x)
        proj_y[jump] = 1
        proj_y = np.cumsum(proj_y, axis=0)

        proj_x = proj_x - 0.5
        mask = np.where(proj_x < 0)
        proj_x[mask] = proj_x[mask] + 1
        ...
        depth = depth[order]  # 按depth从大到小排列
