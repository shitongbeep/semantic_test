from penet.basic import SparseDownSampleClose, CSPNGenerate, CSPN, CSPNGenerateAccelerate, CSPNAccelerate, GeometryFeature, BasicBlockGeo
from penet.basic import weights_init, convbnrelu, deconvbnrelu, convbn, kernel_trans
...
class ENet(nn.Module):
    ...
    def forward(self, input):
        # independent input
        rgb = input['rgb_img'].float()
        d = input['velodyne_proj_img'].float()

        position = input['position'].float()
        K = input['K'].float()
        unorm = position[:, 0:1, :, :]
        vnorm = position[:, 1:2, :, :]
